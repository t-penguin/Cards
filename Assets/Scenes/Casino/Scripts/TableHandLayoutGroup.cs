...
/// <summary>
/// Stripped version of Grid Layout Group made to specifically use center alignment and center anchors
/// </summary>
public class TableHandLayoutGroup : CasinoLayoutGroup
{
    /// <summary>
    ...
    protected TableHandLayoutGroup()
    { }
    ...
    public override void CalculateLayoutInputHorizontal()
    {
        ...
        else
        {
            ...
            int cellCountX = Mathf.Max(1, Mathf.FloorToInt((width + spacing.x + 0.001f) / (cellSize.x + spacing.x)));
            preferredColumns = Mathf.Min(cellCountX, rectChildren.Count);
        }

        SetLayoutInputForAxis(
            (cellSize.x + spacing.x) * minColumns - spacing.x,
            (cellSize.x + spacing.x) * preferredColumns - spacing.x,
            0);
    }
    ...
    public override void CalculateLayoutInputVertical()
    {
        ...
        float minSpace = (cellSize.y + spacing.y) * minRows - spacing.y;
    ...
    private void SetCellsAlongAxis(int axis)
    {
        ...
        else
        {
            if (cellSize.x + spacing.x <= 0)
                ...
                cellCountX = Mathf.Max(1, Mathf.FloorToInt((width + spacing.x + 0.001f) / (cellSize.x + spacing.x)));

            if (cellSize.y + spacing.y <= 0)
                cellCountY = int.MaxValue;
            else
                cellCountY = Mathf.Max(1, Mathf.FloorToInt((height + spacing.y + 0.001f) / (cellSize.y + spacing.y)));
        }
